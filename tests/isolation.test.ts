...
describe("dynamic page guards", () => {
  ...
  it("compares the record's owner to the signed-in user before rendering", () => {
    ...
    for (const page of pages) {
      ...
      // SQL already filters `WHERE id = $1 AND "userId" = $2`, so a mismatched
      // owner comes back as "not found" rather than someone else's row. Both
      // enforce the same guarantee - only the first pattern used to be
      // recognized here, which made e.g. applications/[id]/challenge/page.tsx
      // and .../interview/page.tsx (both use idiom (b) via
