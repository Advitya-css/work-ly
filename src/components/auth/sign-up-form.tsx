...
function GoogleSignUp({ disabled }: { disabled: boolean }) {
  const searchParams = useSearchParams();
  const ref = searchParams?.get("ref");
  return (
    <Button asChild variant="outline" className="w-full" disabled={disabled}>
      <a href={ref ? `/api/auth/google?ref=${encodeURIComponent(ref)}` : "/api/auth/google"}>
  ...
  return (
    <form action={formAction} className="flex flex-col gap-4" noValidate>
      ...
      <Suspense fallback={null}>
        <GoogleSignUp disabled={pending} />
      </Suspense>
      <p className="-mt-2 text-center text-xs text-muted-foreground">
        By continuing with Google you agree to the{" "}
        <Link href="/legal/terms" className="underline hover:text-foreground">Terms</Link> and{" "}
        <Link href="/legal/privacy" className="underline hover:text-foreground">Privacy Policy</Link> and confirm you are 18 or older.
      </p>
