...
export function normalizeToken(s: string): string {
  ...
  // No trailing \b on the c++ pattern: "+" is a non-word character, so a
  // word boundary can never occur right after it (both "+" and the
  // following space/end-of-string count as "non-word" to \b, and a
  // boundary needs one side to be a word character). A trailing \b here
  // meant this replacement never fired for literal "C++", silently leaving
  // it as "c++" instead of "cplusplus" everywhere normalizeToken is used.
  val = val.replace(/\bc\+\+/g, "cplusplus");
